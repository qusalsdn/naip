...
export default function Calendar (){
  ...
  const bizData = [
    ...
    { id: 6,
      ...
      startMonth: 2,
      endMonth: 2,
      Tooltip: "(지역혁신 선도기업) 2024년 지역특화산업육성+(R&D)(제주) 지원계획 통합공고 (과제접수기간) 2024. 02. 05 ~ 2024. 02. 19",
      state: "receptionPrev",
    },
    { id: 7,
      year: "2024",
      ...
      startMonth: 2,
      endMonth: 3,
      Tooltip: "(자유공모) 2024년 지역특화산업육성+(R&D)(제주) 지원계획 통합공고 (과제접수기간) 2024. 02. 05 ~ 2024. 02. 19	",
      state: "receptionPrev",
    },
  ];
    ...
    <article className="calendar">
        ...
        <div className="calendar_table_wrap">
            <table>
                ...
                <thead>
                    <tr className="thead1">
                        <th rowSpan={2} colSpan={2} className="title">사업명 / 공고명</th>
                        <th colSpan={12} className="monthly">월별계획</th>
                    </tr>
                    <tr className="thead2">
                        <th>1월</th>
                        <th>2월</th>
                  ...
                  <tr key={data.id} className="tbody1">
                    ...
                    <td colSpan={12} className="progressBar">
                      {data.state === "receptionPrev" && (
                        <div className={`additionalDivForReceptionPrev ${data.startMonth === 1 ? 'first' : ''}`}
                        style={{
                          position: 'relative',
                          left: `${
                            data.startMonth === 1
                              ? calculateWidthAndLeft(data.startMonth, data.endMonth).left
                              : `calc(${calculateWidthAndLeft(data.startMonth, data.endMonth).left}px - 65px)`
                          }`,
                      ...
                      <div     
                          ...
                          >
                         {tooltips[index] && (
                         <span className={`${data.startMonth >= 7 ? 'right' : ''}`} style={{zIndex : '99999999'}}>
                            {data.Tooltip}
                         </span>
                       )}
                      </div>
