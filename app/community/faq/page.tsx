...
const FAQItem: React.FC<FAQItemProps> = ({ category, questionText, answerText, selectedCategory, toggleCategory }) => {
  ...
  const shouldDisplay = !selectedCategory || selectedCategory === 'null' || category === selectedCategory;
  ...
  shouldDisplay && (
    <div className="">
      <div className={`faqflex ${isOpen ? 'active' : ''}`}>
        ...
        <div onClick={toggleAccordion} className="question"> <span>Q</span>. {questionText}</div>
  ...
  const [selectedCategory, setSelectedCategory] = useState(null);
    const handleCategoryClick = (category : any) => {
      ...
      const faqData = [
        ...
        {
          category: '회원정보',
          questionText: ' 회원가입 시 추가정보에 입력한 관심 키워드, 관심 기관, 관심 금액이란 무엇인가요?',
          answerText: `<span>A</span>. <br>
        ...
        {
          category: '회원정보',
          questionText: ' 회원가입 시 추가정보에 입력한 관심 키워드, 관심 기관, 관심 금액은 수정이 가능한가요?',
          answerText: `<span>A</span>. <br>
        ...
        <div className="filter_bar">
            <ul>
                <li className={` ${selectedCategory === null ? 'active' : ''}`}
                  onClick={() => handleCategoryClick(null)}>전체보기</li>
                <li className={` ${selectedCategory === '공고검색' ? 'active' : ''}`}
                  onClick={() => handleCategoryClick('공고검색')}>공고검색</li>
                  <li className={` ${ selectedCategory === '뉴스레터' ? 'active' : ''}`}
                  onClick={() => handleCategoryClick('뉴스레터')}>뉴스레터</li>
                <li className={` ${ selectedCategory === '용어정리' ? 'active' : ''}`}
                  onClick={() => handleCategoryClick('용어정리')}>용어정리</li>
                <li className={` ${selectedCategory === '회원정보' ? 'active' : ''}`}
                  onClick={() => handleCategoryClick('회원정보')}>회원정보</li>
                <li className={` ${selectedCategory === '기타' ? 'active' : ''}`}
                  onClick={() => handleCategoryClick('기타')}>기타</li>
            </ul>
        </div>
